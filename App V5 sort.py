...
def switch_main_window():
    ...
    cursor.execute('SELECT * FROM USER WHERE user_name=? AND user_password=?', (user_name, user_password))
...
def successful_submit():
    ...
    c7 = '@' and 'gmail.com' in email2_entry.get()
    if c1 and c2 and c3 and c4 and c6 and c7 and not c5:
        # Show SignUp Successfully Window
        signup_success_window.deiconify()
        error2_label.config(text='')
        database_signup()
    elif c4 and c3 and c2 and c1 and not c6:
        error2_label.config(text='Minimum 8 characters of password')
    elif c4 and c3 and c2 and c6 and not c1:
        error2_label.config(text='Password does not match')
    elif c4 and c3 and c2 and c1 and c6 and not c7:
        error2_label.config(text='Invalid email format')
...
welcome_window.iconphoto(False, tk.PhotoImage(file='C:/Users/xueer/OneDrive/Desktop/ALL1(4006CEM)/Tickify_Icon16.png'))
...
signup_success_window.iconphoto(False,
...
main_window.iconphoto(False, tk.PhotoImage(file='C:/Users/xueer/OneDrive/Desktop/ALL1(4006CEM)/Tickify_Icon16.png'))
...
def show_task4():
    ...
    for task in current_tasks:
        ...
        if task[5] is None:
            row_index += 1
            task_frame.grid(row=row_index, column=1)
        ...
        def create_widget(frame, task_created_time):
            def delete_task4():
                if task_title_entry.cget('fg') == '#8D93AB':
                    frame.destroy()
                else:
                    cursor.execute('''SELECT task_title FROM TASK WHERE user_id = ?''', (user_id,))
                    all_task = cursor.fetchall()

                    if (task_title_entry.get(),) in all_task:
                        created_time = task_created_time
                        task_title = task_title_entry.get()
                        cursor.execute(
                            '''SELECT task_id FROM TASK WHERE task_title = ? and user_id = ? and created_time = ? ''',
            ...
            def color_choose4():
                ...
                frame.config(bg=color_for_frame)
                ...
                if hour.isdigit() and len(hour) == 2 and minutes.isdigit() and len(minutes) == 2:
                    ...
                    if 0 <= int_hour <= 23 and 0 <= int_minutes <= 59:
                        ...
                        if current_datetime > reminder_datetime:
                            ...
                            def show_reminder_window(title):
                                reminder_window.deiconify()
                                reminder_window_label1.config(text='!!  Reminder  !!')
                                reminder_window_label2.config(text=title)

                            reminder_window.after(int(millisecond_delay),
                                                  lambda title=task_title_entry.get(): show_reminder_window(title))
                else:
            ...
            def close_reminder_window():
                ...
                cursor.execute(
                    '''SELECT task_id FROM TASK WHERE user_id = ? and created_time = ? ''',
                    (user_id, created_time))
                task_row = cursor.fetchone()
                if task_row:
                    task_id = task_row[0]
                    cursor.execute(
                        '''UPDATE REMINDER SET reminder_date = NULL, reminder_time = NULL where task_id = ?''',
                        (task_id,))
                    connect.commit()
            ...
            def complete_task():
                ...
                if checkbox_var.get() == 1:
                    ...
                    frame.configure(bg=color_plate_button.cget('bg'))
            ...
            delete_button = tk.Button(frame, border=0, bg=task[7], image=delete4_tk_image,
                                      command=delete_task4)
            delete_button.place(x=230, y=9)

            color_plate_button = tk.Button(frame, border=0, bg=task[7], image=color_plate4_tk_image,
                                           command=color_choose4)
            color_plate_button.place(x=210, y=10)

            task_title_entry = tk.Entry(frame, bg=task[7], fg='#363636', border=0, width=24,
                                        font=('Arial', 10, 'bold'))
            task_title_entry.place(x=32, y=8)
            ...
            checkbox = tk.Checkbutton(frame, bg=task[7], variable=checkbox_var, command=complete_task)
            ...
            priority_level_button = tk.Button(frame, border=0, bg=task[7], command=show_priority_level_frame)
            priority_level_button.place(x=9, y=35)
            priority_level_label = tk.Label(frame, bg=task[7], font=('Arial', 7))
            ...
            due_date_button = tk.Button(frame, border=0, bg=task[7], command=show_due_date_frame)
            due_date_button.place(x=9, y=55)
            due_date_label = tk.Label(frame, bg=task[7], font=('Arial', 7))
            due_date_label.place(x=29, y=55)
            if task[5] is None:
                due_date_button.config(image=due_date_grey4_tk_image)
                due_date_label.config(text='Due Date', fg='grey')
            elif task[5] is not None:
                due_date_button.config(image=due_date_black4_tk_image)
                due_date_label.config(text=task[5], fg='black')

            cursor.execute('''Select * from REMINDER where task_id = ?''', (task[0],))
            reminder_value = cursor.fetchall()
            reminder_button = tk.Button(frame, border=0, bg=task[7], command=show_reminder_frame)
            reminder_button.place(x=9, y=75)
            reminder_date_label = tk.Label(frame, bg=task[7], font=('Arial', 7))
            reminder_date_label.place(x=29, y=75)
            reminder_time_label = tk.Label(frame, bg=task[7], font=('Arial', 7))
            reminder_time_label.place(x=99, y=75)

            for reminder in reminder_value:
                if reminder[2] is None and reminder[3] is None:
                    reminder_button.config(image=clock_grey4_tk_image)
                    reminder_date_label.config(text='Date', fg='grey')
                    reminder_time_label.config(text='Time', fg='grey')
                elif reminder[2] is not None and reminder[3] is not None:
                    reminder_button.config(image=clock_black4_tk_image)
                    reminder_date_label.config(text=reminder[2], fg='black')
                    reminder_time_label.config(text=reminder[3], fg='black')

            description_frame = tk.Frame(frame, bg=task[7], width=231, height=140)
            description_frame.place(x=9, y=95)

            description_text = tk.Text(description_frame, bg=task[7], width=35, height=9, fg='#8D93AB',
                                       font=('Arial', 8))
            ...
            edit_done_button = tk.Button(frame, border=0, bg=task[7], image=pencil4_tk_image, text=' Edit',
                                         font=('Arial', 7), compound=tk.LEFT, fg='grey', command=edit_done_task)
            edit_done_button.place(x=208, y=235)
            ...
            reminder_window.iconphoto(False,
                                      tk.PhotoImage(
                                          file='C:/Users/xueer/OneDrive/Desktop/ALL1(4006CEM)/Tickify_Icon16.png'))
        ...
        create_widget(task_frame, task[8])
...
def add_task4():
    ...
    def delete_task4():
        if task_title_entry.cget('fg') == '#8D93AB':
            ...
            cursor.execute('''SELECT task_title FROM TASK WHERE user_id = ? and task_status != ?''',
                           (user_id, 'deleted'))
            all_task = cursor.fetchall()
            ...
            if (task_title_entry.get(),) in all_task:
                ...
                cursor.execute('''SELECT task_id FROM TASK 
                               WHERE task_title = ? and user_id = ? and created_time = ? ''',
                               (task_title, user_id, created_time))
                task_id = cursor.fetchone()[0]
    ...
    def database_task():
        ...
        cursor.execute('''SELECT task_id FROM TASK WHERE user_id = ? and created_time = ? ''',
                       (user_id, created_time))
        task_row = cursor.fetchone()
        if task_row:
            task_id = task_row[0]
...
username4_label = tk.Label(heading4_frame, text='Username', font=("Arial", 12, 'bold'),
                           bg='#EBFAF2', fg='#1C1D22')
